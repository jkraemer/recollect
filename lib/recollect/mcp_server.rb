# frozen_string_literal: true

require "mcp"

module Recollect
  module MCPServer
    TOOLS = [
      Tools::StoreMemory,
      Tools::SearchMemory,
      Tools::GetContext,
      Tools::ListProjects,
      Tools::DeleteMemory
    ].freeze

    PROMPTS = [
      Prompts::SessionLog,
      Prompts::ResumeSession
    ].freeze

    class << self
      def build(db_manager, memories_service: nil)
        memories_service ||= MemoriesService.new(db_manager)
        MCP::Server.new(
          name: "recollect",
          version: Recollect::VERSION,
          tools: TOOLS,
          prompts: PROMPTS,
          resources: Resources::Projects.build(db_manager: db_manager, memories_service: memories_service),
          resource_templates: [Resources::Memory.build(db_manager: db_manager, memories_service: memories_service)],
          server_context: {db_manager: db_manager, memories_service: memories_service},
          configuration: MCP::Configuration.new(validate_tool_call_results: true)
        )
      end
    end
  end
end
